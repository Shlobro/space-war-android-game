...
- `LevelRepositoryTest.kt`: Verifies JSON parsing, validation failures, star-threshold ordering and maximum rules, repository failure reporting for malformed files, catalog sorting, unlock rules, and mapping from level definitions into runtime match state.
