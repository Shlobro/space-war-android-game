...
- `GameTapLaunchMessageTest.kt`: Verifies tap-triggered attacks and friendly reinforcements keep ship-count launch messaging without the redundant batch-status banner, and covers both single-source and multi-source selections so combined launches report a truthful total ship count.
