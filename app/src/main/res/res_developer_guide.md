...
- `values/`: Centralized text and scalar resources referenced by Kotlin and the manifest. `strings.xml` carries short HUD labels plus pluralized gameplay reward copy used by Compose, while `colors.xml` and `themes.xml` stay focused on Android resource consumers.
