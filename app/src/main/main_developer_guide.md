...
- Keep authored campaign metadata such as star thresholds in the shared level JSON so the runtime and editor stay aligned, and keep player progression persistence plus spendable star-currency bookkeeping inside the game package rather than scattering it through UI files.
