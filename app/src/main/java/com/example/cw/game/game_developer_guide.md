...
- `GameScreens.kt`: Menu-only UI for the home screen, asset-backed level selection, and the upgrade screen. These screens surface ready-to-spend stars separately from lifetime earned stars, each level card derives a validated `LevelCardState` before reading the shared palette and status label helpers so impossible combinations such as "completed but locked" are rejected at the helper boundary, and the upgrade screen remains the only place that spends meta-progression currency.
- `GameInGameUi.kt`: In-match HUD, pause overlay, level-end overlay, the selected-base upgrade affordance, and small in-game UI formatting helpers. The top HUD keeps the player economy readout as a compact floating gold `$` amount at the left edge, a live mission timer on its own centered chip directly below the funds/pause row using the same `m:ss` formatter as the results overlay, and an icon-only pause action with no surrounding chip background; that pause control keeps explicit button semantics even though it no longer uses a `Card`. Shared HUD card surfaces now come from `GameUiKit.kt` instead of inline literals, while reward text comes from Android plural resources. Node upgrades trigger a five-second gold ring-fill animation around the upgraded base; the trigger logic is derived from the selected player base's `capLevel` transition instead of being stored as a separate gameplay event, and the animation runner always clears its overlay state in `finally` so selection changes cannot leave a frozen ring behind.
- `GameCanvas.kt`: Custom canvas rendering for the starfield, obstacles, bases, fleet trails, and fleets. Base labels show current unit count in the center and render the base level in a circular badge that overlaps the node's bottom edge using a relative badge offset so the geometry stays readable without competing with the garrison count; the badge radius and stroke width scale from the rendered node radius, while the numeral size clamps using density-aware `sp` bounds converted to pixels inside the composable so the level number stays legible across device densities. When a base is near the bottom of the viewport, the badge clamps upward enough to remain visible. Selection feedback stays visual through the aura and stroke treatment, without a separate text tag above the node.
- `GameUiKit.kt`: Shared visual tokens and reusable Compose primitives such as the menu background, divider, action buttons, stat rows, and the named surface colors reused by the level-card palette helpers. Keep new cross-screen color tokens here rather than scattering raw hex literals through individual composables or styling helpers.
