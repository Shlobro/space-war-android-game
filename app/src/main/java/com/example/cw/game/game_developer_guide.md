...
- `GameInGameUi.kt`: In-match HUD, pause overlay, level-end overlay, the selected-base upgrade affordance, and small in-game UI formatting helpers. The top HUD now focuses on player funds plus an icon-based pause action, and the pause overlay includes restart, resume, and quit actions.
