...
- `GameScreens.kt`: Menu-only UI for the home screen, asset-backed level selection, and the upgrade screen. These screens surface ready-to-spend stars separately from lifetime earned stars, each level card shows the best earned star row, uses the shared level-card contrast palette plus a status pill to keep copy readable against the space-themed surfaces, and the upgrade screen remains the only place that spends meta-progression currency.
...
- `LevelCardStyle.kt`: Shared palette and status helpers for campaign level cards. This isolates the readable text/background combinations for completed, unlocked, and locked cards so `GameScreens.kt` can stay declarative while tests pin the contrast-oriented state mapping.
